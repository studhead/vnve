...
class Tableform
{
	...
	public function ModifyRecord()
    {
		...
		$html .=  sprintf('<h2>' . prana_Ptext('mod','%s wijzigen') . '</h2>',$this->single);
	...
	*/
	...
	public function ExportRecords()
	{
		...
		$export .= '</tr>';
		$pb = $dbio->ReadRecords(array("table"=>$this->table,"columns"=>$this->columns,"prefilter"=>$this->prefilter,"filters"=>$this->filters));
		foreach ( $pb as $p )
		{
			$export .= '<tr>';
			foreach ($this->allcolumns as $c)
